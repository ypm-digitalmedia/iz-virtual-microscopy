...
function loadData(record) {
	...
	var jqxhr = $.getJSON(url, function (data) {
		...
		.done(function (data) {
			...
			// console.log("sqldata:");
			// console.log(sqldata);
			targetSqlData = _.findLast(sqldata, function (d) {
				return d.catalog_number == record
			});
			// console.log(targetCdsData);

			var irns = {
				slider: targetSqlData.media_sliders_irns == "" ? [] : targetSqlData.media_sliders_irns.split("|"),
				zoomify: targetSqlData.media_zoomify_irns == "" ? [] : targetSqlData.media_zoomify_irns.split("|")
			};
			// console.log(irns);
			...
			_.forEach(irns.slider, function (SI, index) {
				...
				repos.push({
					...
					type: "slider",
					obj: foundSlider,
					thumbnail: foundSlider.derivatives["2"].source,
					irn: SI,
					catalogNum: targetSqlData.catalog_number
				});
			});
			...
			_.forEach(irns.zoomify, function (ZI, index) {
				...
				repos.push({
					...
					type: "zoomify",
					obj: foundZoomify,
					thumbnail: foundZoomify.derivatives["2"].source,
					irn: ZI,
					catalogNum: targetSqlData.catalog_number
				});
				repos = _.uniq(repos);
