...
//    $sql = "SELECT * FROM emu_metadata WHERE catalog_number = '" . $vars_arr['catalogNum'] ."' LIMIT 1";
    ...
    if ($result->num_rows > 0) {
        ...
        while($row = $result->fetch_assoc()) {
			...
			if( $row["media_zoomify_irns"] != "" ) {
				$zoomifyList = explode("|",$row["media_zoomify_irns"]);
				for( $z=0; $z < count($zoomifyList); $z++ ) {
					$zPos = strpos($zoomifyList[$z], $theIrn);
					if( $zPos !== false ) { 
						$zIdx = $z;
					}
				}
	...
	// $folder_zstack = 'Annotations/zstack/'
...
?>
		...
		?>
				...
				if(showAnnotations) {
					...
					// new - NO annotations
					Z.showImage("myContainer", imagefolderlocation, "zMarkupMode=0&zMagnifierVisible=1&zMaskVisible=1&zLogoVisible=0");
				}

				// new - editing mode
				// Z.showImage("myContainer", imagefolderlocation, "zMarkupMode=1&zEditMode=0&zAnnotationPath=Annotations/zoomify/annotation_" + slide + ".xml&zFileHandlerPath=Assets/FileHandlers/files.php&zMagnifierVisible=1&zMaskVisible=1&zLogoVisible=0&zAnnotationsAutoSave=1");
	...
	<body class="presentation_zoomify">
		<div id="everything">
			...
			<div id="controls" class="controls-zoomify container-fluid">
				...
				<div class="row">
					...
					<div class="col-xs-5" id="specimenInfo">
						...
						<?php if(is_file($file)) : ?>
							<div id="showAnnoSwitchContainer"><span class="meta-label meta-label-annoswitch"><i class="fas fa-comments"></i>&nbsp;Annotations: </span><input type="checkbox" class="toggle" data-toggle="toggle" checked data-size="mini" id="showAnnoSwitch" /></div>
						<?php endif; ?>
					</div>
				</div>
			...
			<footer><div id="<?=$randomfive?>" class="ab"></div></footer>
